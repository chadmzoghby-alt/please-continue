...
export function resolveTreeSitterPaths({
  cwd = process.cwd(),
  isPackaged = Boolean((process as typeof process & { pkg?: unknown }).pkg),
  moduleDir = __dirname,
  nodeEnv = process.env.NODE_ENV,
}: TreeSitterPathOptions = {}) {
  const useRepositoryTestPaths = nodeEnv === "test" && !isPackaged;

  return {
    queryDirectory: useRepositoryTestPaths
      ? path.join(cwd, "..", "extensions", "vscode", "tree-sitter")
      : path.join(moduleDir, "..", "tree-sitter"),
    wasmDirectory: useRepositoryTestPaths
      ? path.join(cwd, "node_modules", "tree-sitter-wasms", "out")
      : path.join(moduleDir, "tree-sitter-wasms"),
  };
}
...
export async function getQueryForFile(
  ...
  const sourcePath = path.join(
    resolveTreeSitterPaths().queryDirectory,
    queryPath,
  );
...
async function loadLanguageForFileExt(
  ...
  const wasmPath = path.join(
    resolveTreeSitterPaths().wasmDirectory,
