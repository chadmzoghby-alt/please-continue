...
export class SqliteDb {
  ...
  private static closingPromise: Promise<void> | null = null;
  ...
  static async get() {
    if (SqliteDb.closingPromise) {
      await SqliteDb.closingPromise;
    }
    ...
    if (SqliteDb.initializationPromise) {
      return SqliteDb.initializationPromise;
    }
    ...
    SqliteDb.initializationPromise = initializationPromise;

    try {
      return await initializationPromise;
    } finally {
      if (SqliteDb.initializationPromise === initializationPromise) {
  ...
  static async close() {
    if (SqliteDb.closingPromise) {
      return SqliteDb.closingPromise;
    }

    const closingPromise = (async () => {
      if (SqliteDb.initializationPromise) {
        try {
          await SqliteDb.initializationPromise;
        } catch {
          return;
        }
      }
    ...
    SqliteDb.closingPromise = closingPromise;

    try {
      await closingPromise;
    } finally {
      if (SqliteDb.closingPromise === closingPromise) {
        SqliteDb.closingPromise = null;
      }
    }
  }
}
...
async function getTagsFromGlobalCache(
  ...
  try {
    return await stmt.all(cacheKey, artifactId);
  } finally {
    await stmt.finalize();
  }
}
