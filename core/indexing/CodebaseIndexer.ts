...
export class CodebaseIndexer {
  ...
  private activeRefreshAbortController: AbortController | undefined;
  private activeRefreshPromise: Promise<void> | undefined;
  private clearIndexesPromise: Promise<void> | undefined;
  ...
  private isActiveRefresh(): boolean {
    return this.activeRefreshPromise !== undefined;
  }

  private async waitForActiveRefresh(): Promise<void> {
    while (this.isActiveRefresh()) {
      const activeRefreshAbortController = this.activeRefreshAbortController;
      const activeRefreshPromise = this.activeRefreshPromise;
  ...
  private async waitForClearIndexes(): Promise<void> {
    while (this.clearIndexesPromise) {
      await this.clearIndexesPromise;
    }
  }

  async clearIndexes() {
    if (this.clearIndexesPromise) {
      await this.waitForClearIndexes();
      return;
    }

    let markClearIndexesSettled!: () => void;
    const clearIndexesPromise = new Promise<void>((resolve) => {
      markClearIndexesSettled = resolve;
    });
    this.clearIndexesPromise = clearIndexesPromise;
    ...
    try {
      ...
      if (this.clearIndexesPromise === clearIndexesPromise) {
        this.clearIndexesPromise = undefined;
      }
      markClearIndexesSettled();
    }
  }
  ...
  protected async getIndexesToBuild(): Promise<CodebaseIndex[]> {
    ...
    const indexTypesToBuild = Array.from(
    ...
    const fullTextSearchIndex = indexTypesToBuild.indexOf("fullTextSearch");
    const chunkIndex = indexTypesToBuild.indexOf("chunk");
    if (fullTextSearchIndex !== -1 && chunkIndex > fullTextSearchIndex) {
      const [chunkIndexType] = indexTypesToBuild.splice(chunkIndex, 1);
      indexTypesToBuild.splice(fullTextSearchIndex, 0, chunkIndexType);
    }
  ...
  public async refreshCodebaseIndex(paths: string[]) {
    ...
    while (this.clearIndexesPromise || this.isActiveRefresh()) {
      if (this.clearIndexesPromise) {
        await this.waitForClearIndexes();
      } else {
        await this.waitForActiveRefresh();
    ...
    this.activeRefreshAbortController = localController;
    let markRefreshSettled!: () => void;
    const activeRefreshPromise = new Promise<void>((resolve) => {
      markRefreshSettled = resolve;
    });
    this.activeRefreshPromise = activeRefreshPromise;
    ...
    try {
      ...
      if (this.activeRefreshAbortController === localController) {
        this.activeRefreshAbortController = undefined;
      }
      if (this.activeRefreshPromise === activeRefreshPromise) {
