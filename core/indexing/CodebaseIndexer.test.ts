...
describe("CodebaseIndexer", () => {
  ...
  describe("Sequential Integration Tests", () => {
    ...
    test("should create git repo for testing", async () => {
      execSync(
        `cd ${TEST_DIR_PATH} && git init && git add -A && git commit -m "First commit" && git branch -M main`,
      );
    });
  ...
  describe("Core.ts Methods (Isolated Unit Tests)", () => {
    ...
    test("does not close the shared SQLite connection while a codebase refresh is unsettled", async () => {
      ...
      expect(SqliteDb.db).toBe(db);

      let refreshSettled = false;
      const refreshPromise = codebaseIndexer.refreshCodebaseIndex([TEST_DIR]);
      ...
      const dbCloseSpy = jest.spyOn(db, "close");
      const clearPromise = codebaseIndexer.clearIndexes();
      ...
      try {
        ...
        releaseRefresh();
        await Promise.allSettled([refreshPromise, clearPromise]);
        dbCloseSpy.mockRestore();
        refreshDirsSpy.mockRestore();
        await SqliteDb.close();
      }
    });

    test("closes the shared SQLite connection before clearing indexes", async () => {
      const db = await SqliteDb.get();
      expect(SqliteDb.db).toBe(db);
      ...
      expect(fs.existsSync(getIndexSqlitePath())).toBe(false);

      const reopenedDb = await SqliteDb.get();
  ...
  describe("Configuration Handling (Isolated Unit Tests)", () => {
    ...
    describe("getIndexesToBuild", () => {
      ...
      test("should build chunks before the dependent full-text index", async () => {
        mockConfig.contextProviders = [
          {
            description: {
              dependsOnIndexing: ["fullTextSearch", "chunk"],
            },
          },
        ];
        ...
        expect(indexes.map(({ artifactId }) => artifactId)).toEqual([
          "chunks",
          "sqliteFts",
        ]);
      });
